...
public class Main {
	...
	public static void main(String[] args) {
		...
		try (AnnotationConfigApplicationContext ctx = new AnnotationConfigApplicationContext(AppConfigProxy.class)) {
		...
		try (AnnotationConfigApplicationContext ctx = new AnnotationConfigApplicationContext(AppConfigNoProxy.class)) {
			B b1 = ctx.getBean(B.class);
			B b2 = ctx.getBean(B.class);
			System.out.println("B1 == B2? " + (b1 == b2));
		}
